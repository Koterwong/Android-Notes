...
- b. 设置android:configChanges="orientation|keyboardHidden|screenSize"时，切屏不会重新调用各个生命周期，只会执行onConfigurationChanged方法。一般适用于全屏播放视频。
- b. 设置android:configChanges="orientation|keyboardHidden|screenSize"时，切屏不会重新调用各个生命周期，只会执行onConfigurationChanged方法。一般适用于全屏播放视频。
...
可以通过`startService(intent)`来启动，该服务会需要时自动创建，当完成任务时自动关闭。
...
2、图片适配：为不同分辨率制作切图mdpi:hdpi:xhdpi:xxhdpi:xxxdpi==2:3:4:6:8。但是一般在布局文件中直接把图片的宽高写死，然后设置scaleType。
3、布局适配：
